...
if (!process.env.REPLIT_DOMAINS) {
  console.warn("Warning: REPLIT_DOMAINS environment variable not provided. This may cause authentication issues in production deployment.");
  
  // In production, we might need to fallback or disable auth
  if (process.env.NODE_ENV === 'production') {
    console.error("CRITICAL: REPLIT_DOMAINS is required for production deployment authentication.");
    throw new Error("Environment variable REPLIT_DOMAINS not provided - required for production authentication");
  }
}
...
export function getSession() {
  ...
  if (!sessionSecret) {
    ...
    console.warn("Warning: SESSION_SECRET not provided, using fallback for development");
  }
...
export async function setupAuth(app: Express) {
  ...
  // Skip auth setup if REPLIT_DOMAINS is not available (graceful degradation)
  if (!process.env.REPLIT_DOMAINS) {
    console.warn("Skipping Replit Auth setup due to missing REPLIT_DOMAINS");
    return;
  }
